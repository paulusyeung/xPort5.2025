...
namespace xPort5.Settings.Coding.Group
{
    public partial class GroupRecord : Form
    {
        ...
        private void SetCaptions()
        {
            ...
            this.Text = string.Format("{0} {1}", string.Format(oDict.GetWord("record"), oDict.GetWord("group")), oDict.GetWordWithSquareBracket(this._EditMode.ToString() + " Mode"));

            lblCode.Text = string.Format(oDict.GetWordWithColon("code_replace"), oDict.GetWord("group"));
            lblName.Text = string.Format(oDict.GetWordWithColon("name4others_replace"), oDict.GetWord("group"));
            lblName_Chs.Text = string.Format(oDict.GetWordWithColon("name_chs_replace"), oDict.GetWord("group"));
            lblName_Cht.Text = string.Format(oDict.GetWordWithColon("name_cht_replace"), oDict.GetWord("group"));
        }
...
//            T_Currency.LoadCombo(ref cboCurrency, "CurrencyCode", false);
        ...
        private void SetAnsToolbar()
        {
            ...
            xPort5.Controls.Utility.ToolbarControl.LoadAnsBaseButtons(ref this.ansToolbar, _EditMode, "Settings", "Coding.Group");
        ...
        #region ShowGroup(), SaveItem(), VerifyItem(), DeleteItem()
        private void ShowGroup()
        {
            T_Group group = T_Group.Load(_GroupId);
            if (group != null)
            {
                txtCode.Text = group.GroupCode;
                txtName.Text = group.GroupName;
                txtName_Chs.Text = group.GroupName_Chs;
                txtName_Cht.Text = group.GroupName_Cht;
            }
        }
                ...
                try
                {
                    ...
                    switch ((int)_EditMode)
                    {
                        case (int)Common.Enums.EditMode.Add:
                            ...
                            break;
                        case (int)Common.Enums.EditMode.Edit:
                            group = T_Group.Load(_GroupId);
                            break;
                    }
                    group.GroupCode = txtCode.Text.Trim();
                    group.GroupName = txtName.Text.Trim();
                    group.GroupName_Chs = txtName_Chs.Text.Trim();
                    group.GroupName_Cht = txtName_Cht.Text.Trim();

                    group.Save();

                    #region log activity
                    if (_EditMode == Common.Enums.EditMode.Add)
                        xPort5.Controls.Log4net.LogInfo(xPort5.Controls.Log4net.LogAction.Create, group.ToString());
                    else
                        xPort5.Controls.Log4net.LogInfo(xPort5.Controls.Log4net.LogAction.Update, group.ToString());
                    #endregion
        ...
        private bool VerifyItem()
        {
            ...
            if (_EditMode == Common.Enums.EditMode.Add)
            {
                ...
                try
                {
                    sql = String.Format("GroupCode = '{0}'", txtCode.Text.Trim());
        ...
        private void cmdSave_Click(object sender, EventArgs e)
        {
            if (((Form)sender).DialogResult == DialogResult.Yes)
            {
                if (!this.Text.Contains("ReadOnly"))
                {
                    if (SaveItem())
                    {
                        MessageBox.Show(String.Format("Group Code {0} is saved!", _GroupCode), "Save Result");
                        if (_EditMode == Common.Enums.EditMode.Add)
                        {
        ...
        private void cmdSaveClose_Click(object sender, EventArgs e)
        {
            if (((Form)sender).DialogResult == DialogResult.Yes)
            {
                if (!this.Text.Contains("ReadOnly"))
                {
                    if (SaveItem())
                    {
                        MessageBox.Show(String.Format("Group Code {0} is saved!", _GroupCode), "Save Result", MessageBoxButtons.OK, MessageBoxIcon.Information, new EventHandler(cmdCloseForm));
                    }
                    else
                    {
                        MessageBox.Show("Error found...Job aborted!\nPlease review your changes.", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        ...
        private void cmdDelete_Click(object sender, EventArgs e)
        {
            if (((Form)sender).DialogResult == DialogResult.Yes)
            {
                if (!this.Text.Contains("ReadOnly"))
                {
                    if (DeleteItem())
                    {
                        MessageBox.Show(String.Format("Group Code {0} is deleted.", _GroupCode), "Information", MessageBoxButtons.OK, MessageBoxIcon.Information, new EventHandler(cmdCloseForm));
                    }
                    else
                    {
