...
namespace xPort5.Settings.Coding.PhoneLabel
{
    public partial class LabelRecord : Form
    {
        ...
        private void SetCaptions()
        {
            ...
            this.Text = string.Format("{0} {1}", string.Format(oDict.GetWord("record"), oDict.GetWord("phone_label")), oDict.GetWordWithSquareBracket(this._EditMode.ToString() + " Mode"));

            lblCode.Text = string.Format(oDict.GetWordWithColon("code_replace"), oDict.GetWord("phone_label"));
            lblName.Text = string.Format(oDict.GetWordWithColon("name4others_replace"), oDict.GetWord("phone_label"));
            lblName_Chs.Text = string.Format(oDict.GetWordWithColon("name_chs_replace"), oDict.GetWord("phone_label"));
            lblName_Cht.Text = string.Format(oDict.GetWordWithColon("name_cht_replace"), oDict.GetWord("phone_label"));
        }

        private void SetAttributes()
        {
            //toolTip1.SetToolTip(txtName_Chs, "Numeric only");
...
//            T_Currency.LoadCombo(ref cboCurrency, "CurrencyCode", false);
        ...
        private void SetAnsToolbar()
        {
            ...
            xPort5.Controls.Utility.ToolbarControl.LoadAnsBaseButtons(ref this.ansToolbar, _EditMode, "Settings", "Coding.PhoneLabel");
        ...
        #region ShowLabel(), SaveItem(), VerifyItem(), DeleteItem()
        private void ShowLabel()
        {
            Z_Phone phone = Z_Phone.Load(_PhoneId);
            if (phone != null)
            {
                txtCode.Text = phone.PhoneCode;
                txtName.Text = phone.PhoneName;
                txtName_Chs.Text = phone.PhoneName_Chs;
                txtName_Cht.Text = phone.PhoneName_Cht;
            }
        }
                ...
                try
                {
                    ...
                    switch ((int)_EditMode)
                    {
                        case (int)Common.Enums.EditMode.Add:
                            ...
                            break;
                        case (int)Common.Enums.EditMode.Edit:
                            phone = Z_Phone.Load(_PhoneId);
                            break;
                    }
                    phone.PhoneCode = txtCode.Text.Trim();
                    phone.PhoneName = txtName.Text.Trim();
                    phone.PhoneName_Chs = txtName_Chs.Text.Trim();
                    phone.PhoneName_Cht = txtName_Cht.Text.Trim();

                    phone.Save();

                    #region log activity
                    if (_EditMode == Common.Enums.EditMode.Add)
                        xPort5.Controls.Log4net.LogInfo(xPort5.Controls.Log4net.LogAction.Create, phone.ToString());
                    else
                        xPort5.Controls.Log4net.LogInfo(xPort5.Controls.Log4net.LogAction.Update, phone.ToString());
                    #endregion

                    if (phone.PhoneId != _PhoneId)
                    {
                        _PhoneId = phone.PhoneId;
                    }
                    _PhoneCode = phone.PhoneCode;
                    result = true;
                }
                catch { }
            }
        ...
        private bool VerifyItem()
        {
            ...
            if (_EditMode == Common.Enums.EditMode.Add)
            {
                ...
                try
                {
                    sql = String.Format("PhoneCode = '{0}'", txtCode.Text.Trim());
        ...
        private void cmdSave_Click(object sender, EventArgs e)
        {
            if (((Form)sender).DialogResult == DialogResult.Yes)
            {
                if (!this.Text.Contains("ReadOnly"))
                {
                    if (SaveItem())
                    {
                        MessageBox.Show(String.Format("Item {0} is saved!", _PhoneCode), "Save Result");
                        if (_EditMode == Common.Enums.EditMode.Add)
                        {
        ...
        private void cmdSaveClose_Click(object sender, EventArgs e)
        {
            if (((Form)sender).DialogResult == DialogResult.Yes)
            {
                if (!this.Text.Contains("ReadOnly"))
                {
                    if (SaveItem())
                    {
                        MessageBox.Show(String.Format("Item {0} is saved!", _PhoneCode), "Save Result", MessageBoxButtons.OK, MessageBoxIcon.Information, new EventHandler(cmdCloseForm));
                    }
                    else
                    {
                        MessageBox.Show("Error found...Job aborted!\nPlease review your changes.", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        ...
        private void cmdDelete_Click(object sender, EventArgs e)
        {
            if (((Form)sender).DialogResult == DialogResult.Yes)
            {
                if (!this.Text.Contains("ReadOnly"))
                {
                    if (DeleteItem())
                    {
                        MessageBox.Show(String.Format("Item {0} is deleted.", _PhoneCode), "Information", MessageBoxButtons.OK, MessageBoxIcon.Information, new EventHandler(cmdCloseForm));
                    }
                    else
                    {
