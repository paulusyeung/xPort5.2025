...
namespace xPort5.Order.Coding.Customer.Address
{
    public partial class AddressList : UserControl
    {
        ...
        private void SetLvwList()
        {
            ...
            lvwAddress.Tag = new Guid("847E1889-BC30-4005-AB50-228B166C5BEC");
        ...
        private void SetAttribute()
        {
            ...
            string[] phones = xPort5.Controls.Utility.Default.PhoneLabels4Customer();
...
SELECT TOP 100 PERCENT
      ...
      ,ISNULL([ModifiedBy], '')
      ,[Retired]                -- 20
  FROM [dbo].[vwCustomerAddressList]
WHERE [CustomerId] = '{0}'
...
", _CustomerId.ToString());
            ...
            while (reader.Read())
            {
                ...
                CustomerAddress cAddress = CustomerAddress.Load(cAddressId);
        ...
        private void ShowRecord()
        {
            if (lvwAddress.SelectedItem != null)
            {
                ...
                if (Common.Utility.IsGUID(lvwAddress.SelectedItem.SubItems[1].Text))
                {
                    ...
                    address.CustomerId = _CustomerId;
                    address.CustomerAddressId = uAddressId;
                    address.ShowDialog();
                }
            }
        }
                ...
                if (lvwAddress.CheckBoxes && lvwAddress.CheckedIndices.Count > 0)
                {
                    foreach (ListViewItem item in lvwAddress.CheckedItems)
                    {
                        if (Common.Utility.IsGUID(item.SubItems[1].Text))
                        {
                            ...
                            if (xPort5.Controls.Utility.CustomerAddress.DeleteRec(aSupplierId))
                            {
                                item.Remove();
                            }
                        }
