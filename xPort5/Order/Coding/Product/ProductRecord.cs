...
namespace xPort5.Order.Coding.Product
{
    public partial class ProductRecord : Form
    {
        ...
        private void SetCaptions()
        {
            ...
            this.Text = string.Format("{0} {1}", string.Format(oDict.GetWord("record"), oDict.GetWord("product")), oDict.GetWordWithSquareBracket(this._EditMode.ToString() + " Mode"));
        ...
        private void ShowProduct()
        {
            ...
            if (item != null)
            {
                ...
                if (item.ColorPattern == String.Empty)
                {
                    ...
                    {
                        if (colorList.Items[i].ToString().Trim() == item.ColorPattern.Trim())
                        {
                ...
                try
                {
                    ...
                    switch ((int)_EditMode)
                    {
                        case (int)Common.Enums.EditMode.Add:
                            ...
                            item.Status = (int)Common.Enums.Status.Active;          // skip Draft stage
                    ...
                    //item.ColorId = (Guid)cboColor.SelectedValue;
                    ...
                    {
                        xPort5.Controls.Utility.Resources.RenamePicture(_ProductId, item.ArticleId);
                        _ProductCode = item.ArticleCode;
                    }
        ...
        private bool VerifyItem()
        {
            ...
            #region validate Catrgory
            if (cboCategory.SelectedValue == null)
            {
                errMsg += Environment.NewLine + "Category cannot be blank.";
                result = false;
            }
            else if (!Common.Utility.IsGUID(cboCategory.SelectedValue.ToString()))
            {
                errMsg += Environment.NewLine + "Category cannot be blank.";
                result = false;
            }
