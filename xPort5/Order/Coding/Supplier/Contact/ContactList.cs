...
namespace xPort5.Order.Coding.Supplier.Contact
{
    public partial class ContactList : UserControl
    {
        ...
        private void SetLvwList()
        {
            ...
            lvwAddress.Tag = new Guid("3E706F6D-87C7-418f-8451-9919D4241ACA");
...
SELECT TOP 100 PERCENT
      ...
      ,ISNULL([ModifiedBy], '')
      ,[Retired]
FROM [dbo].[vwSupplierContactList]
WHERE [SupplierId] = '{0}'
ORDER BY [FullName]
", _SupplierId.ToString());
            ...
            while (reader.Read())
            {
                ...
                SupplierContact cContact = SupplierContact.Load(cContactId);
        ...
        private void ShowRecord()
        {
            if (lvwAddress.SelectedItem != null)
            {
                ...
                if (Common.Utility.IsGUID(lvwAddress.SelectedItem.SubItems[1].Text))
                {
                    ...
                    contact.SupplierId = _SupplierId;
                    contact.SupplierContactId = uAddressId;
                    contact.ShowDialog();
                }
            }
        }
                ...
                if (lvwAddress.CheckBoxes && lvwAddress.CheckedIndices.Count > 0)
                {
                    foreach (ListViewItem item in lvwAddress.CheckedItems)
                    {
                        if (Common.Utility.IsGUID(item.SubItems[1].Text))
                        {
                            ...
                            if (xPort5.Controls.Utility.SupplierContact.DeleteRec(aSupplierId))
                            {
                                item.Remove();
                            }
                        }
