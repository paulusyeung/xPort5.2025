...
namespace xPort5.Order.Coding.Supplier.Address
{
    public partial class AddressList : UserControl
    {
        ...
        private void SetCaptions()
        {
            ...
            this.colAddressName.Text = oDict.GetWord("addresstype");
            this.colAddress.Text = oDict.GetWord("address");
            this.colPhone1.Text = oDict.GetWord("business");
            this.colPhone2.Text = oDict.GetWord("business") + 2;
            this.colPhone3.Text = oDict.GetWord("fax");
            this.colPhone4.Text = oDict.GetWord("website");
        ...
        private void SetAttribute()
        {
            ...
            string[] phones = xPort5.Controls.Utility.Default.PhoneLabels4Supplier();
        ...
        private void SetPackageAns()
        {
            ...
            {
                if (xPort5.Controls.Utility.NetSqlAzMan.IsAccessAuthorized("Order", "Coding.Supplier.Create"))
                {
            ...
            if (_EditMode == Common.Enums.EditMode.Edit)
            {
                if (xPort5.DAL.Common.Config.UseNetSqlAzMan)
                {
                    if (xPort5.Controls.Utility.NetSqlAzMan.IsAccessAuthorized("Order", "Coding.Supplier.Delete"))
                    {
...
SELECT TOP 100 PERCENT
      ...
      ,ISNULL([ModifiedBy], '')
      ,[Retired]                -- 20
  FROM [dbo].[vwSupplierAddressList]
WHERE [SupplierId] = '{0}'
...
", _SupplierId.ToString());
            ...
            while (reader.Read())
            {
                Guid cAddressId = reader.GetGuid(0);
                SupplierAddress cAddress = SupplierAddress.Load(cAddressId);

                ListViewItem objItem = this.lvwAddress.Items.Add(iCount.ToString());  // Line Number
                #region Address Image
                if (cAddress.DefaultRec)
                {
                    objItem.SmallImage = new IconResourceHandle("16x16.addresssingle_key_16.png");
                    objItem.LargeImage = new IconResourceHandle("16x16.addresssingle_key_16.png");
                }
                else
                {
        ...
        private void ShowRecord()
        {
            if (lvwAddress.SelectedItem != null)
            {
                ...
                if (Common.Utility.IsGUID(lvwAddress.SelectedItem.SubItems[1].Text))
                {
                    ...
                    address.SupplierId = _SupplierId;
                    address.SupplierAddressId = uAddressId;
                    address.ShowDialog();
                }
            }
        }
                ...
                if (lvwAddress.CheckBoxes && lvwAddress.CheckedIndices.Count > 0)
                {
                    foreach (ListViewItem item in lvwAddress.CheckedItems)
                    {
                        if (Common.Utility.IsGUID(item.SubItems[1].Text))
                        {
                            ...
                            if (xPort5.Controls.Utility.SupplierAddress.DeleteRec(aSupplierId))
                            {
                                item.Remove();
                            }
                        }
