...
namespace xPort5.Order.Coding.Supplier.Address
{
    public partial class AddressRecord : Form
    {
        ...
        private Guid _SupplierAddressId = System.Guid.Empty;
        ...
        {
            string[] phoneLabels = xPort5.Controls.Utility.Default.PhoneLabels4Supplier();
        ...
        {
            xPort5.DAL.Supplier customer = xPort5.DAL.Supplier.Load(_SupplierId);
            if (customer != null)
            {
                txtUserCode.Text = customer.SupplierCode;
                txtUserAlias.Text = customer.SupplierName;
            }
        }

        private void ShowSupplierAddress()
        {
            SupplierAddress uAddress = SupplierAddress.Load(_SupplierAddressId);
            if (uAddress != null)
            {
        ...
        private bool SaveItem()
        {
            ...
            if (VerifyItem())
            {
                try
                {
                    SupplierAddress uAddress = null;
                    switch ((int)_EditMode)
                    {
                        case (int)Common.Enums.EditMode.Add:
                            uAddress = new SupplierAddress();
                            uAddress.SupplierId = _SupplierId;
                            ...
                            if (xPort5.Controls.Utility.SupplierAddress.Count(_SupplierId) == 0)
                            {
                            ...
                            Guid[] labelIds = xPort5.Controls.Utility.Default.PhoneLabelIds4Supplier();
                            ...
                            break;
                        case (int)Common.Enums.EditMode.Edit:
                            uAddress = SupplierAddress.Load(_SupplierAddressId);
                            uAddress.ModifiedOn = DateTime.Now;
                            uAddress.ModifiedBy = Common.Config.CurrentUserId;
                            break;
                    }
                    ...
                    if (uAddress.SupplierAddressId != _SupplierAddressId)
                    {
                        _SupplierAddressId = uAddress.SupplierAddressId;
                    }
                    result = true;
                }
                catch { }
        ...
        private bool VerifyItem()
        {
            ...
            if (_EditMode == Common.Enums.EditMode.Add)
            {
                ...
                try
                {
                    ...
                    sql = String.Format("SupplierId = '{0}' AND AddressId = '{1}'",
                        _SupplierId.ToString(),
                        addr.AddressId.ToString());
                    xPort5.DAL.SupplierAddress cAddress = xPort5.DAL.SupplierAddress.LoadWhere(sql);
                    if (cAddress != null)
                    {
                ...
                {
                    errMsg += Environment.NewLine + "Supplier + Address is invalid.";
                    result = false;
                }
                #endregion
            }
        ...
        {
            return xPort5.Controls.Utility.CustomerAddress.DeleteRec(_SupplierAddressId);
        }
        #endregion
                ...
                if (!this.Text.Contains("ReadOnly"))
                {
                    if (SaveItem())
                    {
                        ...
                        if (_EditMode == Common.Enums.EditMode.Edit)
                        {
                            ...
                            _SupplierAddressId = System.Guid.NewGuid();
