...
{
    public partial class PreOrderListXr_Pdf : DevExpress.XtraReports.UI.XtraReport
    {
        public PreOrderListXr_Pdf()
        {
            InitializeComponent();
        ...
        private void Detail_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            this.txtCostPrice2.Text = GetCurrentColumnValue("OrderedCny").ToString() + " " + Convert.ToDecimal(GetCurrentColumnValue("QuotedUnitAmt")).ToString("n2") + " / " + GetCurrentColumnValue("OrderedUnit").ToString();
            ...
            #region bind subreport CustShipment
            ShipmentSchedule_Customer CustShipment = new ShipmentSchedule_Customer();
            DataTable dtCustShipment = xPort5.Controls.Reporting.XR.DataSource.PreOrderListCustShipment(GetCurrentColumnValue("PLNumber").ToString(), GetCurrentColumnValue("PLLineNo").ToString());
            CustShipment.DataSource = dtCustShipment;
            this.subCustShipment.ReportSource = CustShipment;
            #endregion

            #region bind subreport SuppShipment
            ShipmentSchedule_Supplier SuppShipment = new ShipmentSchedule_Supplier();
            DataTable dtSuppShipment = xPort5.Controls.Reporting.XR.DataSource.PreOrderListSuppShipment(GetCurrentColumnValue("PLNumber").ToString(), GetCurrentColumnValue("PLLineNo").ToString());
            SuppShipment.DataSource = dtSuppShipment;
            this.subSuppShipment.ReportSource = SuppShipment;
            #endregion
        }
