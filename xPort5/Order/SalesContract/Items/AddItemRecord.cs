...
SELECT [QTNumber]           --0
  ...
  FROM [dbo].[vwOrderQTItemList]";
                ...
                while (reader.Read())
                {
                    ...
                    lvItem.SubItems.Add(reader.GetString(9));   // Customer Ref.
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            if (lvItemList.CheckedItems.Count > 0)
            {
                foreach (ListViewItem lvItem in lvItemList.CheckedItems)
                {
                    ...
                    {
                        string sql = "OrderQTItemId = '" + lvItem.Text + "' AND OrderSCId = '" + this.OrderId.ToString() + "'";
                        OrderSCItems item = OrderSCItems.LoadWhere(sql);
                        if (item == null)
                        {
                            item = new OrderSCItems();
                            item.OrderSCId = this.OrderId;
        ...
        private void lvItemList_Click(object sender, EventArgs e)
        {
