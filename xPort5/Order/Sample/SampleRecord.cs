...
{
    public partial class SampleRecord : Form
    {
        ...
        private void SetAttributes()
        {
            ...
            {
                case (int)Common.Enums.EditMode.Add:
                    cmdNextSPNumber.Visible = true;
                    txtSPNumber.Width = txtSPNumber.Width - 25;
                    wspBottom.Enabled = false;
                    break;
                case (int)Common.Enums.EditMode.Edit:
                    txtSPNumber.Enabled = false;
                    cmdNextSPNumber.Visible = false;
                    wspBottom.Enabled = true;
                    break;
            }

            this.Text = string.Format("{0} {1}", string.Format(oDict.GetWord("record"), oDict.GetWord("sample")), oDict.GetWordWithSquareBracket(this._EditMode.ToString() + " Mode"));

            dtpDate.Format = DateTimePickerFormat.Custom;
            dtpDate.CustomFormat = Common.DateTimeHelper.GetDateFormat();

            lblPreOrderNumber.Text = oDict.GetWordWithColon("sample_no");
            ...
            toolTip1.SetToolTip(cmdNextSPNumber, string.Format(oDict.GetWord("generate_number"), oDict.GetWord("sample_no")));
        }
        ...
        private void SetAnsToolbar()
        {
            ...
            xPort5.Controls.Utility.ToolbarControl.LoadAnsBaseButtons(ref this.ansToolbar, _EditMode, "Order", "Order.Sample");
            ...
            if (e.Button.Tag != null)
            {
                switch (e.Button.Tag.ToString().ToLower())
                {
                    ...
                    case "upload":
                        ...
                        attachWizard.OrderId = this._SampleId;
                        attachWizard.OrderNumber = this._SPNumber;
        ...
        private bool SaveItem()
        {
            ...
            if (VerifyItem())
            {
                try
                {
                    ...
                    item.SPNumber = txtSPNumber.Text.Trim();
                    item.SPDate = dtpDate.Value;
                    ...
                    if (item.CustomerId != _SampleId)
                    {
                        _SampleId = item.OrderSPId;
                        _SPNumber = item.SPNumber;
                    }
                    result = true;
                }
        ...
        private bool VerifyItem()
        {
            ...
            if (_EditMode == Common.Enums.EditMode.Add)
            {
                ...
                if (txtSPNumber.Text.Trim() == String.Empty)
                {
                    errMsg += Environment.NewLine + "Sample No. cannot be blank.";
                    result = false;
                }
                else
                {
        ...
        {
            return xPort5.Controls.Utility.OrderSP.DeleteRec(_SampleId);
        }
        ...
        private void cmdSave_Click(object sender, EventArgs e)
        {
            if (((Form)sender).DialogResult == DialogResult.Yes)
            {
                if (!this.Text.Contains("ReadOnly"))
                {
                    if (SaveItem())
                    {
                        MessageBox.Show(String.Format("Pre-Order {0} is saved!", _SPNumber), "Save Result");
                        if (_EditMode == Common.Enums.EditMode.Add)
                        {
        ...
        private void cmdSaveClose_Click(object sender, EventArgs e)
        {
            if (((Form)sender).DialogResult == DialogResult.Yes)
            {
                if (!this.Text.Contains("ReadOnly"))
                {
                    if (SaveItem())
                    {
                        MessageBox.Show(String.Format("Customer Code {0} is saved!", _SPNumber), "Save Result", MessageBoxButtons.OK, MessageBoxIcon.Information, new EventHandler(cmdCloseForm));
                    }
                    else
                    {
                        MessageBox.Show("Error found...Job aborted!\nPlease review your changes.", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        ...
        private void cmdSaveDup_Click(object sender, EventArgs e)
        {
            if (((Form)sender).DialogResult == DialogResult.Yes)
            {
                if (!this.Text.Contains("ReadOnly"))
                {
                    if (SaveItem())
                    {
                        MessageBox.Show(String.Format("Customer Code {0} is saved!", _SPNumber), "Save Result");
        ...
        private void cmdDelete_Click(object sender, EventArgs e)
        {
            if (((Form)sender).DialogResult == DialogResult.Yes)
            {
                if (!this.Text.Contains("ReadOnly"))
                {
                    if (DeleteItem())
                    {
                        MessageBox.Show(String.Format("Customer Code {0} is deleted.", _SPNumber), "Information", MessageBoxButtons.OK, MessageBoxIcon.Information, new EventHandler(cmdCloseForm));
                    }
                    else
                    {
        ...
        private void cmdNextSPNumber_Click(object sender, EventArgs e)
        {
            txtSPNumber.Text = xPort5.Controls.Utility.OrderSP.NextSPNumber();
        }
    }
}