...
namespace xPort5.Order.Sample.Items
{
    public partial class ItemList : UserControl
    {
        ...
        private void SetAttribute()
        {
            ...
            this.colQTLineNumber.Text = oDict.GetWord("qt_line_number");
        }
...
", _SampleId.ToString());
        ...
        void imgItem_DoubleClick(object sender, EventArgs e)
        {
            ...
            //OrderSPItems item = OrderSPItems.LoadWhere(String.Format("OrderSPItemsId = '{0}'", prodImage.Name));
            //if (item != null)
            //{
            //    ItemRecord itemRecord = new ItemRecord();

            //    itemRecord.EditMode = Common.Enums.EditMode.Edit;
            //    itemRecord.SampleId = item.OrderSPId;
            //    itemRecord.SampleItemId = item.OrderSPItemsId;
            ...
            if (!string.IsNullOrEmpty(e.Button.Name))
            {
                switch (e.Button.Name.ToLower())
                {
                    ...
                    case "new":
                        ...
                        itemRecord.EditMode = Common.Enums.EditMode.Add;
                        itemRecord.SampleId = this._SampleId;
                        itemRecord.NextLineNumber = this.lvwItems.Items.Count + 1;
                        itemRecord.ExceptQTNumber = GetSelectedQTNumber();
                        itemRecord.ShowDialog();
                        break;
                    case "popup":
                        ShowRecord();
        ...
        private void ShowRecord()
        {
            if (lvwItems.SelectedItem != null)
            {
                if (lvwItems.SelectedItem != null)
                {
                    ...
                    //    item.SampleId = _SampleId;
                    //    item.SampleItemId = itemId;
                    //    item.ShowDialog();
                    //}
                }
            }
        ...
        private string GetSelectedQTNumber()
        {
            string item = "'{0}'";
            ...
            foreach (ListViewItem lvItem in lvwItems.Items)
            {
                ...
                result.Append(string.Format(item, lvItem.SubItems[1].Text.Trim()));
            }

            if (result.Length == 0)
            {
                result.Append(string.Format(item, string.Empty));
            }
            ...
            if (((Form)sender).DialogResult == DialogResult.Yes)
            {
                if (lvwItems.Visible)
                {
                    if (lvwItems.CheckBoxes && lvwItems.CheckedIndices.Count > 0)
                    {
                        foreach (ListViewItem item in lvwItems.CheckedItems)
                        {
                            ...
                            {
                                Guid spItemId = new Guid(item.SubItems[1].Text);

                                if (xPort5.Controls.Utility.OrderSP.DeleteItem(spItemId))
                                {
                                    item.Remove();
                                }
                            }
                    ...
                    else
                    {
                        if (lvwItems.SelectedIndex >= 0)
                        {
                            if (Common.Utility.IsGUID(lvwItems.SelectedItem.SubItems[1].Text))
                            {
                                Guid spItemId = new Guid(lvwItems.SelectedItem.SubItems[1].Text);
                ...
                else
                {
                    ...
                    if (selectedList.Count > 0)
                    {
                        ...
                        {
                            xPort5.Controls.Utility.OrderSP.DeleteItem(spItemId);
                        }
                    }
        ...
        private void ansImageList_MenuClick(object sender, MenuItemEventArgs e)
        {
            ...
            {
                case "Small":
                    BindImageList(new Size(90, 65), false);
                    break;
                case "Medium":
                    BindImageList(new Size(120, 90), false);
                    break;
                case "Large":
                    BindImageList(new Size(240, 180), false);
                    break;
                case "Details":
                    BindImageList(new Size(320, 240), true);
                    break;
            }
        }
