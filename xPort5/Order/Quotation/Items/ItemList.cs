...
namespace xPort5.Order.Quotation.Items
{
    public partial class ItemList : UserControl
    {
        ...
        private Guid _QuotationId = System.Guid.Empty;
        private string _QTNumber = String.Empty;
...
", _QuotationId.ToString());
        ...
        private void BindImageList(Size imageSize, bool inDetail)
        {
            ...
            while (reader.Read())
            {
                ...
                if (item != null)
                {
                    ImagePanel imgItem = new ImagePanel(imageSize, item.ArticleId, inDetail, itemId, System.Guid.Empty, string.Empty, lvwItems.CheckBoxes, false);
        ...
        void imgItem_DoubleClick(object sender, EventArgs e)
        {
            ...
            OrderQTItems item = OrderQTItems.LoadWhere(String.Format("OrderQTItemId = '{0}'", prodImage.Name));
            if (item != null)
            {
                ...
                itemRecord.OrderId = item.OrderQTId;
                itemRecord.OrderItemId = item.OrderQTItemId;
        ...
        private void ShowRecord()
        {
            if (lvwItems.SelectedItem != null)
            {
                ...
                {
                    Guid orderItemId = new Guid(lvwItems.SelectedItem.SubItems[1].Text);
                    ...
                    item.LineNumber = int.Parse(lvwItems.SelectedItem.SubItems[2].Text);
                    item.ShowDialog();
                }
            }
        }
            ...
            if (((Form)sender).DialogResult == DialogResult.Yes)
            {
                    ...
                    if (lvwItems.CheckBoxes && lvwItems.CheckedIndices.Count > 0)
                    {
                        foreach (ListViewItem item in lvwItems.CheckedItems)
                        {
                            if (Common.Utility.IsGUID(item.SubItems[1].Text))
                            {
                                ...
                                if (xPort5.Controls.Utility.OrderQT.DeleteItem(itemId))
                                {
                                    item.Remove();
                ...
                else
                {
                    ...
                    if (selectedList.Count > 0)
                    {
                        ...
                        {
                            result = xPort5.Controls.Utility.OrderQT.DeleteItem(qtItemId);
                        }
                    }
                ...
                if (!result)
                {
                    MessageBox.Show("Item is in use.", "Error found", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
        #endregion
