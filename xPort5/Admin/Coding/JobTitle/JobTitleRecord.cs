...
namespace xPort5.Admin.Coding.JobTitle
{
    public partial class JobTitleRecord : Form
    {
        ...
        {
            this.Text = string.Format("{0} {1}", string.Format(oDict.GetWord("record"), oDict.GetWord("job_title")), oDict.GetWordWithSquareBracket(this._EditMode.ToString() + " Mode"));

            lblCode.Text = string.Format(oDict.GetWordWithColon("code_replace"), oDict.GetWord("job_title"));
            lblName.Text = string.Format(oDict.GetWordWithColon("name4others_replace"), oDict.GetWord("job_title"));
            lblName_Chs.Text = string.Format(oDict.GetWordWithColon("name_chs_replace"), oDict.GetWord("job_title"));
            lblName_Cht.Text = string.Format(oDict.GetWordWithColon("name_cht_replace"), oDict.GetWord("job_title"));
        }
...
//            T_Currency.LoadCombo(ref cboCurrency, "CurrencyCode", false);
        ...
        private void SetAnsToolbar()
        {
            ...
            xPort5.Controls.Utility.ToolbarControl.LoadAnsBaseButtons(ref this.ansToolbar, _EditMode, "Admin", "Coding.JobTitle");
        ...
        {
            Z_JobTitle item = Z_JobTitle.Load(_JobTitleId);
            if (item != null)
            {
                txtCode.Text = item.JobTitleCode;
                txtName.Text = item.JobTitleName;
                txtName_Chs.Text = item.JobTitleName_Chs;
                txtName_Cht.Text = item.JobTitleName_Cht;
            }
        }
        ...
        private bool SaveItem()
        {
            ...
            if (VerifyItem())
            {
                try
                {
                    Z_JobTitle item = null;
                    switch ((int)_EditMode)
                    {
                        case (int)Common.Enums.EditMode.Add:
                            item = new Z_JobTitle();
                            item.JobTitleId = _JobTitleId;
                            break;
                        case (int)Common.Enums.EditMode.Edit:
                            item = Z_JobTitle.Load(_JobTitleId);
                            break;
                    }
                    item.JobTitleCode = txtCode.Text.Trim();
                    item.JobTitleName = txtName.Text.Trim();
                    item.JobTitleName_Chs = txtName_Chs.Text.Trim();
                    item.JobTitleName_Cht = txtName_Cht.Text.Trim();
                    ...
                    if (item.JobTitleId != _JobTitleId)
                    {
                        _JobTitleId = item.JobTitleId;
                    }
                    _JobTitleCode = item.JobTitleCode;
                    result = true;
                }
                catch { }
            }
        ...
        private bool VerifyItem()
        {
            ...
            if (_EditMode == Common.Enums.EditMode.Add)
            {
                ...
                try
                {
                    sql = String.Format("JobTitleCode = '{0}'", txtCode.Text.Trim());
        ...
        private void cmdSave_Click(object sender, EventArgs e)
        {
            if (((Form)sender).DialogResult == DialogResult.Yes)
            {
                if (!this.Text.Contains("ReadOnly"))
                {
                    if (SaveItem())
                    {
                        MessageBox.Show(String.Format("Item {0} is saved!", _JobTitleCode), "Save Result");
                        if (_EditMode == Common.Enums.EditMode.Add)
                        {
        ...
        private void cmdSaveClose_Click(object sender, EventArgs e)
        {
            if (((Form)sender).DialogResult == DialogResult.Yes)
            {
                if (!this.Text.Contains("ReadOnly"))
                {
                    if (SaveItem())
                    {
                        MessageBox.Show(String.Format("Item {0} is saved!", _JobTitleCode), "Save Result", MessageBoxButtons.OK, MessageBoxIcon.Information, new EventHandler(cmdCloseForm));
                    }
                    else
                    {
                        MessageBox.Show("Error found...Job aborted!\nPlease review your changes.", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                ...
                if (!this.Text.Contains("ReadOnly"))
                {
                    if (SaveItem())
                    {
                        ...
                        if (_EditMode == Common.Enums.EditMode.Edit)
                        {
                            ...
                            _JobTitleId = System.Guid.NewGuid();
                            _JobTitleCode = string.Empty;
        ...
        private void cmdDelete_Click(object sender, EventArgs e)
        {
            if (((Form)sender).DialogResult == DialogResult.Yes)
            {
                if (!this.Text.Contains("ReadOnly"))
                {
                    if (DeleteItem())
                    {
                        MessageBox.Show(String.Format("Item {0} is deleted.", _JobTitleCode), "Information", MessageBoxButtons.OK, MessageBoxIcon.Information, new EventHandler(cmdCloseForm));
                    }
                    else
                    {
