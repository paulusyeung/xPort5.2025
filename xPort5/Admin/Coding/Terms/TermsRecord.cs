...
namespace xPort5.Admin.Coding.Terms
{
    public partial class TermsRecord : Form
    {
        ...
        private string _TermsCode = String.Empty;
        ...
        {
            this.Text = string.Format("{0} {1}", string.Format(oDict.GetWord("record"), oDict.GetWord("payment_terms")), oDict.GetWordWithSquareBracket(this._EditMode.ToString() + " Mode"));

            rbtnReceipts.Text = String.Format("R = {0}", oDict.GetWord("customer"));
            rbtnPayment.Text = String.Format("P = {0}", oDict.GetWord("supplier"));

            lblCode.Text = string.Format(oDict.GetWordWithColon("code_replace"), oDict.GetWord("payment_terms"));
            lblName.Text = oDict.GetWord("payment_terms");
            lblType.Text = oDict.GetWordWithColon("type");
            lblName_Chs.Text = string.Format(oDict.GetWordWithColon("chs_replace"), oDict.GetWord("payment_terms"));
            lblName_Cht.Text = string.Format(oDict.GetWordWithColon("cht_replace"), oDict.GetWord("payment_terms"));
        }
        ...
        private void SetAnsToolbar()
        {
            ...
            xPort5.Controls.Utility.ToolbarControl.LoadAnsBaseButtons(ref this.ansToolbar, _EditMode, "Admin", "Coding.PaymentTerms");
        ...
        {
            T_PaymentTerms item = T_PaymentTerms.Load(_TermsId);
            if (item != null)
            {
                txtCode.Text = item.TermsCode;

                rbtnPayment.Checked = (item.TermsType == "P");
                rbtnReceipts.Checked = (item.TermsType == "R");

                txtName.Text = item.TermsName;
                txtName_Chs.Text = item.TermsName_Chs;
                txtName_Cht.Text = item.TermsName_Cht;
            }
        }
        ...
        private bool SaveItem()
        {
            ...
            if (VerifyItem())
            {
                try
                {
                    T_PaymentTerms item = null;
                    switch ((int)_EditMode)
                    {
                        case (int)Common.Enums.EditMode.Add:
                            item = new T_PaymentTerms();
                            item.TermsId = _TermsId;
                            break;
                        case (int)Common.Enums.EditMode.Edit:
                            item = T_PaymentTerms.Load(_TermsId);
                            break;
                    }
                    item.TermsCode = txtCode.Text.Trim();
                    item.TermsType = rbtnPayment.Checked ? rbtnPayment.Tag.ToString() : rbtnReceipts.Tag.ToString();
                    item.TermsName = txtName.Text.Trim();
                    item.TermsName_Chs = txtName_Chs.Text.Trim();
                    item.TermsName_Cht = txtName_Cht.Text.Trim();
                    ...
                    if (item.TermsId != _TermsId)
                    {
                        _TermsId = item.TermsId;
                    }
                    _TermsCode = item.TermsCode;
                    result = true;
                }
                catch { }
            }
        ...
        private bool VerifyItem()
        {
            ...
            if (_EditMode == Common.Enums.EditMode.Add)
            {
                ...
                try
                {
                    sql = String.Format("TermsCode = '{0}'", txtCode.Text.Trim());
                    T_PaymentTerms group = T_PaymentTerms.LoadWhere(sql);
                    if (group != null)
                    {
        ...
        private bool DeleteItem()
        {
            ...
            try
            {
                ...
                T_PaymentTerms.Delete(_TermsId);
                result = true;
            }
            catch { }
            return result;
        ...
        private void cmdSave_Click(object sender, EventArgs e)
        {
            if (((Form)sender).DialogResult == DialogResult.Yes)
            {
                if (!this.Text.Contains("ReadOnly"))
                {
                    if (SaveItem())
                    {
                        MessageBox.Show(String.Format("Item {0} is saved!", _TermsCode), "Save Result");
                        if (_EditMode == Common.Enums.EditMode.Add)
                        {
        ...
        private void cmdSaveClose_Click(object sender, EventArgs e)
        {
            if (((Form)sender).DialogResult == DialogResult.Yes)
            {
                if (!this.Text.Contains("ReadOnly"))
                {
                    if (SaveItem())
                    {
                        MessageBox.Show(String.Format("Item {0} is saved!", _TermsCode), "Save Result", MessageBoxButtons.OK, MessageBoxIcon.Information, new EventHandler(cmdCloseForm));
                    }
                    else
                    {
                        MessageBox.Show("Error found...Job aborted!\nPlease review your changes.", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                ...
                if (!this.Text.Contains("ReadOnly"))
                {
                    if (SaveItem())
                    {
                        ...
                        if (_EditMode == Common.Enums.EditMode.Edit)
                        {
                            ...
                            _TermsId = System.Guid.NewGuid();
                            _TermsCode = string.Empty;
        ...
        private void cmdDelete_Click(object sender, EventArgs e)
        {
            if (((Form)sender).DialogResult == DialogResult.Yes)
            {
                if (!this.Text.Contains("ReadOnly"))
                {
                    if (DeleteItem())
                    {
                        MessageBox.Show(String.Format("Item {0} is deleted.", _TermsCode), "Information", MessageBoxButtons.OK, MessageBoxIcon.Information, new EventHandler(cmdCloseForm));
                    }
                    else
                    {
