...
namespace xPort5.Admin.Coding.ColorTable
{
    public partial class ColorRecord : Form
    {
        nxStudio.BaseClass.WordDict oDict = new nxStudio.BaseClass.WordDict(Common.Config.CurrentWordDict, Common.Config.CurrentLanguageId);

        private Common.Enums.EditMode _EditMode = Common.Enums.EditMode.Read;
        private Guid _ColorId = System.Guid.Empty;
        ...
        {
            this.Text = string.Format("{0} {1}", string.Format(oDict.GetWord("record"), oDict.GetWord("color")), oDict.GetWordWithSquareBracket(this._EditMode.ToString() + " Mode"));

            lblCode.Text = string.Format(oDict.GetWordWithColon("code_replace"), oDict.GetWord("color"));
            lblName.Text = string.Format(oDict.GetWordWithColon("name4others_replace"), oDict.GetWord("color"));
            lblName_Chs.Text = string.Format(oDict.GetWordWithColon("name_chs_replace"), oDict.GetWord("color"));
            lblName_Cht.Text = string.Format(oDict.GetWordWithColon("name_cht_replace"), oDict.GetWord("color"));
        }
...
//            T_Currency.LoadCombo(ref cboCurrency, "CurrencyCode", false);
        ...
        private void SetAnsToolbar()
        {
            ...
            xPort5.Controls.Utility.ToolbarControl.LoadAnsBaseButtons(ref this.ansToolbar, _EditMode, "Admin", "Coding.Color");
        ...
        {
            T_Color item = T_Color.Load(_ColorId);
            if (item != null)
            {
                txtCode.Text = item.ColorCode;
                txtName.Text = item.ColorName;
                txtName_Chs.Text = item.ColorName_Chs;
                txtName_Cht.Text = item.ColorName_Cht;
            }
        }
            ...
            if (VerifyItem())
            {
                try
                {
                    ...
                    switch ((int)_EditMode)
                    {
                        case (int)Common.Enums.EditMode.Add:
                            item = new T_Color();
                            item.ColorId = _ColorId;
                            break;
                        case (int)Common.Enums.EditMode.Edit:
                            item = T_Color.Load(_ColorId);
                            break;
                    }
                    item.ColorCode = txtCode.Text.Trim();
                    item.ColorName = txtName.Text.Trim();
                    item.ColorName_Chs = txtName_Chs.Text.Trim();
                    item.ColorName_Cht = txtName_Cht.Text.Trim();
                    ...
                    if (item.ColorId != _ColorId)
                    {
                        _ColorId = item.ColorId;
                    }
                    _ColorCode = item.ColorCode;
        ...
        private bool VerifyItem()
        {
            ...
            if (_EditMode == Common.Enums.EditMode.Add)
            {
                ...
                try
                {
                    sql = String.Format("ColorName = N'{0}'", txtName.Text.Trim());
                    T_Color item = T_Color.LoadWhere(sql);
                    if (item != null)
                    {
                        errMsg += Environment.NewLine + "Color is in use.";
                        result = false;
                    }
                }
                catch
                {
                    errMsg += Environment.NewLine + "Color is invalid.";
                    result = false;
                }
                #endregion
            }
        ...
        private void cmdSave_Click(object sender, EventArgs e)
        {
            if (((Form)sender).DialogResult == DialogResult.Yes)
            {
                if (!this.Text.Contains("ReadOnly"))
                {
                    if (SaveItem())
                    {
                        MessageBox.Show(String.Format("Item {0} is saved!", _ColorName), "Save Result");
                        if (_EditMode == Common.Enums.EditMode.Add)
                        {
        ...
        private void cmdSaveClose_Click(object sender, EventArgs e)
        {
            if (((Form)sender).DialogResult == DialogResult.Yes)
            {
                if (!this.Text.Contains("ReadOnly"))
                {
                    if (SaveItem())
                    {
                        MessageBox.Show(String.Format("Item {0} is saved!", _ColorName), "Save Result", MessageBoxButtons.OK, MessageBoxIcon.Information, new EventHandler(cmdCloseForm));
                    }
                    else
                    {
                        MessageBox.Show("Error found...Job aborted!\nPlease review your changes.", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        ...
        private void cmdDelete_Click(object sender, EventArgs e)
        {
            if (((Form)sender).DialogResult == DialogResult.Yes)
            {
                if (!this.Text.Contains("ReadOnly"))
                {
                    if (DeleteItem())
                    {
                        MessageBox.Show(String.Format("Item {0} is deleted.", _ColorName), "Information", MessageBoxButtons.OK, MessageBoxIcon.Information, new EventHandler(cmdCloseForm));
                    }
                    else
                    {
