...
namespace xPort5.Admin.Coding.ShippingMark
{
    public partial class ShippingMarkRecord : Form
    {
        nxStudio.BaseClass.WordDict oDict = new nxStudio.BaseClass.WordDict(Common.Config.CurrentWordDict, Common.Config.CurrentLanguageId);

        private Common.Enums.EditMode _EditMode = Common.Enums.EditMode.Read;
        private Guid _ShippingMarkId = System.Guid.Empty;
        private string _ShippingMarkCode = String.Empty;

        public ShippingMarkRecord()
        {
            InitializeComponent();
        }
        ...
        public Guid ShippingMarkId
        {
            get
            {
                return _ShippingMarkId;
            }
            set
            {
                _ShippingMarkId = value;
            }
        }
        #endregion
        ...
        {
            this.Text = string.Format("{0} {1}", string.Format(oDict.GetWord("record"), oDict.GetWord("shipping_mark")), oDict.GetWordWithSquareBracket(this._EditMode.ToString() + " Mode"));

            lblCode.Text = string.Format(oDict.GetWordWithColon("code_replace"), oDict.GetWord("shipping_mark"));
            lblName.Text = oDict.GetWord("shipping_mark");
            lblName_Chs.Text = string.Format(oDict.GetWordWithColon("chs_replace"), oDict.GetWord("shipping_mark"));
            lblName_Cht.Text = string.Format(oDict.GetWordWithColon("cht_replace"), oDict.GetWord("shipping_mark"));
        }
...
//            T_Currency.LoadCombo(ref cboCurrency, "CurrencyCode", false);
        ...
        private void SetAnsToolbar()
        {
            ...
            xPort5.Controls.Utility.ToolbarControl.LoadAnsBaseButtons(ref this.ansToolbar, _EditMode, "Admin", "Coding.ShippingMark");
        ...
        {
            T_ShippingMark item = T_ShippingMark.Load(_ShippingMarkId);
            if (item != null)
            {
                txtCode.Text = item.ShippingMarkCode;
                txtName.Text = item.ShippingMarkName;
                txtName_Chs.Text = item.ShippingMarkName_Chs;
                txtName_Cht.Text = item.ShippingMarkName_Cht;
            }
        }
        ...
        private bool SaveItem()
        {
            ...
            if (VerifyItem())
            {
                try
                {
                    T_ShippingMark item = null;
                    switch ((int)_EditMode)
                    {
                        case (int)Common.Enums.EditMode.Add:
                            item = new T_ShippingMark();
                            item.ShippingMarkId = _ShippingMarkId;
                            break;
                        case (int)Common.Enums.EditMode.Edit:
                            item = T_ShippingMark.Load(_ShippingMarkId);
                            break;
                    }
                    item.ShippingMarkCode = txtCode.Text.Trim();
                    item.ShippingMarkName = txtName.Text.Trim();
                    item.ShippingMarkName_Chs = txtName_Chs.Text.Trim();
                    item.ShippingMarkName_Cht = txtName_Cht.Text.Trim();
                    ...
                    if (item.ShippingMarkId != _ShippingMarkId)
                    {
                        _ShippingMarkId = item.ShippingMarkId;
                    }
                    _ShippingMarkCode = item.ShippingMarkCode;
                    result = true;
                }
                catch { }
            }
        ...
        private bool VerifyItem()
        {
            ...
            if (_EditMode == Common.Enums.EditMode.Add)
            {
                ...
                try
                {
                    sql = String.Format("ShippingMarkCode = '{0}'", txtCode.Text.Trim());
        ...
        private bool DeleteItem()
        {
            ...
            try
            {
                ...
                T_ShippingMark.Delete(_ShippingMarkId);
                result = true;
            }
            catch { }
            return result;
        ...
        private void cmdSave_Click(object sender, EventArgs e)
        {
            if (((Form)sender).DialogResult == DialogResult.Yes)
            {
                if (!this.Text.Contains("ReadOnly"))
                {
                    if (SaveItem())
                    {
                        MessageBox.Show(String.Format("Item {0} is saved!", _ShippingMarkCode), "Save Result");
                        if (_EditMode == Common.Enums.EditMode.Add)
                        {
        ...
        private void cmdSaveClose_Click(object sender, EventArgs e)
        {
            if (((Form)sender).DialogResult == DialogResult.Yes)
            {
                if (!this.Text.Contains("ReadOnly"))
                {
                    if (SaveItem())
                    {
                        MessageBox.Show(String.Format("Item {0} is saved!", _ShippingMarkCode), "Save Result", MessageBoxButtons.OK, MessageBoxIcon.Information, new EventHandler(cmdCloseForm));
                    }
                    else
                    {
                        MessageBox.Show("Error found...Job aborted!\nPlease review your changes.", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                ...
                if (!this.Text.Contains("ReadOnly"))
                {
                    if (SaveItem())
                    {
                        ...
                        if (_EditMode == Common.Enums.EditMode.Edit)
                        {
                            ...
                            _ShippingMarkId = System.Guid.NewGuid();
                            _ShippingMarkCode = string.Empty;
        ...
        private void cmdDelete_Click(object sender, EventArgs e)
        {
            if (((Form)sender).DialogResult == DialogResult.Yes)
            {
                if (!this.Text.Contains("ReadOnly"))
                {
                    if (DeleteItem())
                    {
                        MessageBox.Show(String.Format("Item {0} is deleted.", _ShippingMarkCode), "Information", MessageBoxButtons.OK, MessageBoxIcon.Information, new EventHandler(cmdCloseForm));
                    }
                    else
                    {
