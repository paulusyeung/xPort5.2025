...
namespace xPort5.Admin.Coding.Salutation
{
    public partial class SalutationRecord : Form
    {
        ...
        private string _SalutationCode = String.Empty;

        public SalutationRecord()
        {
            InitializeComponent();
        }
        ...
        {
            this.Text = string.Format("{0} {1}", string.Format(oDict.GetWord("record"), oDict.GetWord("salutation")), oDict.GetWordWithSquareBracket(this._EditMode.ToString() + " Mode"));

            lblCode.Text = string.Format(oDict.GetWordWithColon("code_replace"), oDict.GetWord("salutation"));
            lblName.Text = oDict.GetWord("salutation");
            lblName_Chs.Text = string.Format(oDict.GetWordWithColon("chs_replace"), oDict.GetWord("salutation"));
            lblName_Cht.Text = string.Format(oDict.GetWordWithColon("cht_replace"), oDict.GetWord("salutation"));
        }
...
//            T_Currency.LoadCombo(ref cboCurrency, "CurrencyCode", false);
        ...
        private void SetAnsToolbar()
        {
            ...
            xPort5.Controls.Utility.ToolbarControl.LoadAnsBaseButtons(ref this.ansToolbar, _EditMode, "Admin", "Coding.Salutation");
        ...
        {
            Z_Salutation item = Z_Salutation.Load(_SalutationId);
            if (item != null)
            {
                txtCode.Text = item.SalutationCode;
                txtName.Text = item.SalutationName;
                txtName_Chs.Text = item.SalutationName_Chs;
                txtName_Cht.Text = item.SalutationName_Cht;
            }
        }
        ...
        private bool SaveItem()
        {
            ...
            if (VerifyItem())
            {
                try
                {
                    Z_Salutation item = null;
                    switch ((int)_EditMode)
                    {
                        case (int)Common.Enums.EditMode.Add:
                            item = new Z_Salutation();
                            item.SalutationId = _SalutationId;
                            break;
                        case (int)Common.Enums.EditMode.Edit:
                            item = Z_Salutation.Load(_SalutationId);
                            break;
                    }
                    item.SalutationCode = txtCode.Text.Trim();
                    item.SalutationName = txtName.Text.Trim();
                    item.SalutationName_Chs = txtName_Chs.Text.Trim();
                    item.SalutationName_Cht = txtName_Cht.Text.Trim();
                    ...
                    if (item.SalutationId != _SalutationId)
                    {
                        _SalutationId = item.SalutationId;
                    }
                    _SalutationCode = item.SalutationCode;
                    result = true;
                }
                catch { }
            }
        ...
        private bool VerifyItem()
        {
            ...
            if (_EditMode == Common.Enums.EditMode.Add)
            {
                ...
                try
                {
                    sql = String.Format("SalutationCode = '{0}'", txtCode.Text.Trim());
        ...
        private bool DeleteItem()
        {
            ...
            try
            {
                ...
                Z_Salutation.Delete(_SalutationId);
                result = true;
            }
            catch { }
            return result;
        ...
        private void cmdSave_Click(object sender, EventArgs e)
        {
            if (((Form)sender).DialogResult == DialogResult.Yes)
            {
                if (!this.Text.Contains("ReadOnly"))
                {
                    if (SaveItem())
                    {
                        MessageBox.Show(String.Format("Item {0} is saved!", _SalutationCode), "Save Result");
                        if (_EditMode == Common.Enums.EditMode.Add)
                        {
        ...
        private void cmdSaveClose_Click(object sender, EventArgs e)
        {
            if (((Form)sender).DialogResult == DialogResult.Yes)
            {
                if (!this.Text.Contains("ReadOnly"))
                {
                    if (SaveItem())
                    {
                        MessageBox.Show(String.Format("Item {0} is saved!", _SalutationCode), "Save Result", MessageBoxButtons.OK, MessageBoxIcon.Information, new EventHandler(cmdCloseForm));
                    }
                    else
                    {
                        MessageBox.Show("Error found...Job aborted!\nPlease review your changes.", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                ...
                if (!this.Text.Contains("ReadOnly"))
                {
                    if (SaveItem())
                    {
                        ...
                        if (_EditMode == Common.Enums.EditMode.Edit)
                        {
                            ...
                            _SalutationId = System.Guid.NewGuid();
                            _SalutationCode = string.Empty;
        ...
        private void cmdDelete_Click(object sender, EventArgs e)
        {
            if (((Form)sender).DialogResult == DialogResult.Yes)
            {
                if (!this.Text.Contains("ReadOnly"))
                {
                    if (DeleteItem())
                    {
                        MessageBox.Show(String.Format("Item {0} is deleted.", _SalutationCode), "Information", MessageBoxButtons.OK, MessageBoxIcon.Information, new EventHandler(cmdCloseForm));
                    }
                    else
                    {
